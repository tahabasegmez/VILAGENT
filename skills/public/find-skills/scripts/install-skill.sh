...
# Find project root by looking for vilagent.code-workspace
find_project_root() {
  local dir="$PWD"
  while [[ "$dir" != "/" ]]; do
    if [[ -f "$dir/vilagent.code-workspace" ]]; then
      echo "$dir"
      return 0
    fi
    dir="$(dirname "$dir")"
...
if [[ -z "$PROJECT_ROOT" ]]; then
  echo "Error: Could not find project root (vilagent.code-workspace not found)"
  exit 1
fi
