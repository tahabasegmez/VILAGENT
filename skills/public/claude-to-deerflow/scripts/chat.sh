#!/usr/bin/env bash
# chat.sh — Send a message to VILAGENT and collect the streaming response.
#
# Usage:
...
#   VILAGENT_URL=http://host:2026 bash chat.sh "hi"   # custom endpoint
#
# Environment variables:
#   VILAGENT_URL          — Unified proxy base URL (default: http://localhost:2026)
#   VILAGENT_GATEWAY_URL  — Gateway API base URL (default: $VILAGENT_URL)
#   VILAGENT_LANGGRAPH_URL — LangGraph API base URL (default: $VILAGENT_URL/api/langgraph)
#
...
GATEWAY_URL="${VILAGENT_GATEWAY_URL:-$VILAGENT_URL}"
LANGGRAPH_URL="${VILAGENT_LANGGRAPH_URL:-$VILAGENT_URL/api/langgraph}"
...
if [ "$HTTP_CODE" = "000" ] || [ "$HTTP_CODE" -ge 400 ]; then
  echo "ERROR: VILAGENT is not reachable at ${GATEWAY_URL} (HTTP ${HTTP_CODE})" >&2
  echo "Make sure VILAGENT is running. Start it with: cd <vilagent-dir> && make dev" >&2
  exit 1
fi
...
if result_messages is not None:
    ...
    for event_type, data_str in events:
        if event_type == "error":
            print(f"ERROR from VILAGENT: {data_str}", file=sys.stderr)
            sys.exit(1)
