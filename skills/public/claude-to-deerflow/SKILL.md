---
name: claude-to-vilagent
description: "Interact with VILAGENT AI agent platform via its HTTP API. Use this skill when the user wants to send messages or questions to VILAGENT for research/analysis, start a VILAGENT conversation thread, check VILAGENT status or health, list available models/skills/agents in VILAGENT, manage VILAGENT memory, upload files to VILAGENT threads, or delegate complex research tasks to VILAGENT. Also use when the user mentions vilagent, deer flow, or wants to run a deep research task that VILAGENT can handle."
---

# VILAGENT Skill

Communicate with a running VILAGENT instance via its HTTP API. VILAGENT is an AI agent platform
built on LangGraph that orchestrates sub-agents for research, code execution, web browsing, and more.

## Architecture

VILAGENT exposes two API surfaces behind an Nginx reverse proxy:

| Service        | Direct Port | Via Proxy                        | Purpose                          |
|----------------|-------------|----------------------------------|----------------------------------|
| Gateway API    | 8001        | `$VILAGENT_GATEWAY_URL`          | REST endpoints and embedded agent runtime |
| LangGraph-compatible API | 8001 | `$VILAGENT_LANGGRAPH_URL`       | Agent threads, runs, streaming   |
...
|-------------------------|------------------------------------------|------------------------------------|
| `VILAGENT_URL`          | `http://localhost:2026`                  | Unified proxy base URL             |
| `VILAGENT_GATEWAY_URL`  | `${VILAGENT_URL}`                        | Gateway API base (models, skills, memory, uploads) |
| `VILAGENT_LANGGRAPH_URL`| `${VILAGENT_URL}/api/langgraph`          | LangGraph API base (threads, runs) |

When making curl calls, always resolve the URL like this:

```bash
# Resolve base URLs from env (do this FIRST before any API call)
VILAGENT_URL="${VILAGENT_URL:-http://localhost:2026}"
VILAGENT_GATEWAY_URL="${VILAGENT_GATEWAY_URL:-$VILAGENT_URL}"
VILAGENT_LANGGRAPH_URL="${VILAGENT_LANGGRAPH_URL:-$VILAGENT_URL/api/langgraph}"
```
...
Verify VILAGENT is running:

```bash
curl -s "$VILAGENT_GATEWAY_URL/health"
```
...
```bash
curl -s -X POST "$VILAGENT_LANGGRAPH_URL/threads" \
...
```bash
curl -s -N -X POST "$VILAGENT_LANGGRAPH_URL/threads/<thread_id>/runs/stream" \
  -H "Content-Type: application/json" \
  -d '{
...
```bash
curl -s "$VILAGENT_GATEWAY_URL/api/models"
```
...
```bash
curl -s "$VILAGENT_GATEWAY_URL/api/skills"
```
...
```bash
curl -s -X PUT "$VILAGENT_GATEWAY_URL/api/skills/<skill_name>" \
  -H "Content-Type: application/json" \
  -d '{"enabled": true}'
```
...
```bash
curl -s "$VILAGENT_GATEWAY_URL/api/agents"
```
...
```bash
curl -s "$VILAGENT_GATEWAY_URL/api/memory"
```
...
```bash
curl -s -X POST "$VILAGENT_GATEWAY_URL/api/threads/<thread_id>/uploads" \
...
```bash
curl -s "$VILAGENT_GATEWAY_URL/api/threads/<thread_id>/uploads/list"
```

### 11. Get Thread History

```bash
curl -s "$VILAGENT_LANGGRAPH_URL/threads/<thread_id>/history"
```

### 12. List Threads

```bash
curl -s -X POST "$VILAGENT_LANGGRAPH_URL/threads/search" \
  -H "Content-Type: application/json" \
...
```bash
bash /path/to/skills/claude-to-vilagent/scripts/chat.sh "Your question here"
```
...
- If health check fails, VILAGENT is not running. Inform the user they need to start it.
