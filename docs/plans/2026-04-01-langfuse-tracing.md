...
**Goal:** Add optional Langfuse observability support to VILAGENT while preserving existing LangSmith tracing and allowing both providers to be enabled at the same time.
...
Run: `cd backend && uv run ruff check packages/harness/vilagent/config/tracing_config.py packages/harness/vilagent/models/factory.py packages/harness/vilagent/tracing`
