...
const ENV_KEYS = [
  "NODE_ENV",
  "VILAGENT_INTERNAL_GATEWAY_BASE_URL",
  "VILAGENT_TRUSTED_ORIGINS",
] as const;
...
describe("getGatewayConfig", () => {
  ...
  beforeEach(() => {
    saved = snapshotEnv();
    setEnv("VILAGENT_INTERNAL_GATEWAY_BASE_URL", undefined);
    setEnv("VILAGENT_TRUSTED_ORIGINS", undefined);
  });
  ...
  test("uses env values verbatim when set, regardless of NODE_ENV", async () => {
    setEnv("NODE_ENV", "production");
    setEnv("VILAGENT_INTERNAL_GATEWAY_BASE_URL", "https://gw.example.com/");
    setEnv(
      "VILAGENT_TRUSTED_ORIGINS",
