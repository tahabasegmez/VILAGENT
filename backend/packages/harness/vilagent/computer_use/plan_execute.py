...
- To fill a multi-field form (e.g. an email's To / Subject / Body), move between fields with an explicit deterministic hotkey step (action_kind="hotkey", requires_vision=false, args.keys="TAB") BETWEEN the type_text steps — never rely on the vision model to click or tab between fields. Pattern: type field 1 -> hotkey TAB -> type field 2 -> hotkey TAB -> type field 3. Use the correct number of TABs for the app's field order.
