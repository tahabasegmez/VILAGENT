...
- Plan the TRANSITIONS between steps, not only the actions. Whenever consecutive steps act on different fields, controls, or regions, insert an explicit deterministic step that moves focus there FIRST — a hotkey step (action_kind="hotkey", requires_vision=false, e.g. args.keys="TAB" to advance to the next field, or arrow keys), or a focus/click step — instead of relying on the vision model to find and click each field. This is generic for any multi-field form or wizard (not one specific app): act on field 1 -> move focus (e.g. hotkey TAB) -> act on field 2 -> move focus -> act on field 3. Pick the correct key(s) for that app's focus order.
