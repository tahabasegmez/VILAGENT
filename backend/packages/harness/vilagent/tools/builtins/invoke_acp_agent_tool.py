...
def _build_mcp_servers() -> dict[str, dict[str, Any]]:
    """Build ACP ``mcpServers`` config from VILAGENT's enabled MCP servers."""
    from vilagent.config.extensions_config import ExtensionsConfig
    from vilagent.mcp.client import build_servers_config
...
def _build_acp_mcp_servers() -> list[dict[str, Any]]:
    ...
    The ACP client expects a list of server objects, while VILAGENT's MCP helper
        ...
        try:
            ...
            async with spawn_agent_process(client, cmd, *args, env=agent_env, cwd=physical_cwd) as (conn, proc):
                ...
                await conn.initialize(
                    ...
                    client_info=Implementation(name="vilagent", title="VILAGENT", version="0.1.0"),
                )
