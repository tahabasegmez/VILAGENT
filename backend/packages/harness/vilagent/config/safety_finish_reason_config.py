...
through ``vilagent.reflection.resolve_variable`` (same loader the
...
class SafetyDetectorConfig(BaseModel):
    ...
    use: str = Field(
        description=("Class path of a SafetyTerminationDetector implementation (e.g. 'vilagent.agents.middlewares.safety_termination_detectors:OpenAICompatibleContentFilterDetector')."),
    )
    config: dict = Field(
