...
Bytedance/vilagent issue #3144: gateway request dependencies resolve
...
STARTUP_ONLY_FIELDS: dict[str, str] = {
    ...
    "computer_use": (
        "computer_use_runtime() constructs the Windows Agent Host once during Gateway startup; live sessions, providers, allowlists, audit storage, and emergency-stop state are not rebuilt on config.yaml edits."
    ),
    "log_level": (
        "apply_logging_level() runs only during app.py startup; it sets the vilagent/app logger levels and may lower root handler thresholds so configured messages can propagate. A freshly reloaded AppConfig does not retrigger it."
    ),
