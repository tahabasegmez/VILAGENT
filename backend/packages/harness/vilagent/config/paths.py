...
from vilagent.config.runtime_paths import runtime_home
...
def _default_local_base_dir() -> Path:
    """Return the caller project's writable VILAGENT state directory."""
    return runtime_home()
...
def _join_host_path(base: str, *parts: str) -> str:
    ...
    path form (for example ``C:\\repo\\backend\\.vilagent``).  Using
...
class Paths:
    """
    Centralized path configuration for VILAGENT application data.
    ...
    BaseDir resolution (in priority order):
        ...
        2. VILAGENT_HOME environment variable
        3. Caller project fallback: `{project_root}/.vilagent`
    """
    ...
    def host_base_dir(self) -> Path:
        ...
        Set VILAGENT_HOST_BASE_DIR to the host-side path that corresponds to this
        ...
        """
        if env := os.getenv("VILAGENT_HOST_BASE_DIR"):
            return Path(env)
    ...
    def base_dir(self) -> Path:
        ...
        if env_home := os.getenv("VILAGENT_HOME"):
