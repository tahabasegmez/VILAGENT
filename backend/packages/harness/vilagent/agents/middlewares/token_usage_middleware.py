...
class TokenUsageMiddleware(AgentMiddleware):
    ...
    def _apply(self, state: AgentState) -> dict | None:
        ...
        try:
            from vilagent.tools.builtins.task_tool import pop_cached_subagent_usage
        except ImportError:
            # Subagent execution is not part of the VILAGENT computer-use
            # surface; without the task tool there is no cached usage to merge.
            pop_cached_subagent_usage = None
        if len(messages) >= 2 and pop_cached_subagent_usage is not None:
            idx = len(messages) - 2
            while idx >= 0:
