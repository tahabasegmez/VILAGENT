...
and the embedded :class:`~vilagent.client.VILAGENTClient`.
...
Usage::
    ...
    from vilagent.runtime.store.provider import get_store, store_context
...
from vilagent.runtime.store._sqlite_utils import ensure_sqlite_parent_dir, resolve_sqlite_conn_str
...
POSTGRES_STORE_INSTALL = (
    "langgraph-checkpoint-postgres is required for the PostgreSQL store. Install the package extra with: pip install 'vilagent-harness[postgres]' (or use: uv sync --all-packages --extra postgres when developing locally)"
)
...
def _sync_store_cm(config) -> Iterator[BaseStore]:
    ...
    :class:`~vilagent.config.checkpointer_config.CheckpointerConfig` instance —
...
def get_store() -> BaseStore:
    ...
    from vilagent.config.app_config import _app_config
