...
Usage (e.g. FastAPI lifespan)::
    ...
    from vilagent.runtime.store import make_store
...
from vilagent.runtime.store.provider import POSTGRES_CONN_REQUIRED, POSTGRES_STORE_INSTALL, SQLITE_STORE_INSTALL, ensure_sqlite_parent_dir, resolve_sqlite_conn_str
...
async def _async_store(config) -> AsyncIterator[BaseStore]:
    ...
    The ``config`` argument is a :class:`vilagent.config.checkpointer_config.CheckpointerConfig`
...
async def make_store(app_config: AppConfig | None = None) -> AsyncIterator[BaseStore]:
    ...
    :func:`vilagent.runtime.checkpointer.async_provider.make_checkpointer` so
