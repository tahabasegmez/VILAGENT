...
from vilagent.config import get_app_config
from vilagent.config.app_config import AppConfig
from vilagent.reflection import resolve_class
from vilagent.tracing import build_tracing_callbacks
...
def _enable_stream_usage_by_default(model_use_path: str, model_settings_from_config: dict) -> None:
    ...
    base URL or client is configured. VILAGENT frequently uses OpenAI-compatible
...
def _normalize_provider_settings(model_use_path: str, model_settings_from_config: dict) -> None:
    """Normalize common provider aliases before constructing LangChain models."""
    if model_use_path == "langchain_google_genai:ChatGoogleGenerativeAI":
        gemini_api_key = model_settings_from_config.pop("gemini_api_key", None)
        api_key = model_settings_from_config.pop("api_key", None)
        if not model_settings_from_config.get("google_api_key"):
            model_settings_from_config["google_api_key"] = gemini_api_key or api_key
        max_tokens = model_settings_from_config.pop("max_tokens", None)
        if max_tokens is not None and "max_output_tokens" not in model_settings_from_config:
            model_settings_from_config["max_output_tokens"] = max_tokens
        return

    google_api_key = model_settings_from_config.pop("google_api_key", None)
    gemini_api_key = model_settings_from_config.pop("gemini_api_key", None)
    alias_api_key = google_api_key or gemini_api_key
    if alias_api_key and not model_settings_from_config.get("api_key"):
        model_settings_from_config["api_key"] = alias_api_key
...
def create_chat_model(name: str | None = None, thinking_enabled: bool = False, *, app_config: AppConfig | None = None, attach_tracing: bool = True, **kwargs) -> BaseChatModel:
    ...
    _normalize_provider_settings(model_config.use, model_settings_from_config)

    # For Codex Responses API models: map thinking mode to reasoning_effort
    from vilagent.models.openai_codex_provider import CodexChatModel
