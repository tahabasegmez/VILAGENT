...
run path resolve it through :func:`vilagent.config.app_config.get_app_config`,
...
from vilagent.persistence.feedback import FeedbackRepository
from vilagent.runtime import RunContext, RunManager, StreamBridge
from vilagent.runtime.events.store.base import RunEventStore
from vilagent.runtime.runs.store.base import RunStore
...
if TYPE_CHECKING:
    ...
    from vilagent.persistence.thread_meta.base import ThreadMetaStore
    from vilagent.runtime import RunRecord
...
def get_config() -> AppConfig:
    ...
    Routes through :func:`vilagent.config.app_config.get_app_config`, which
    ...
    :func:`get_app_config` closes the bytedance/vilagent issue #3107 BUG-001
    ...
    :mod:`vilagent.config.reload_boundary` and is mirrored by the
    ...
    *startup-time* run-events configuration the underlying ``event_store``
    ...
    Usage in ``app.py``::
        ...
        async with langgraph_runtime(app, startup_config):
            yield
    """
    from vilagent.persistence.engine import close_engine, get_session_factory, init_engine_from_config
    from vilagent.runtime import make_store, make_stream_bridge
    from vilagent.runtime.checkpointer.async_provider import make_checkpointer
    from vilagent.runtime.events.store import make_run_event_store
    ...
    async with AsyncExitStack() as stack:
        ...
        if sf is not None:
            from vilagent.persistence.feedback import FeedbackRepository
            from vilagent.persistence.run import RunRepository

            app.state.run_store = RunRepository(sf)
            app.state.feedback_repo = FeedbackRepository(sf)
        else:
            from vilagent.runtime.runs.store.memory import MemoryRunStore

            app.state.run_store = MemoryRunStore()
            app.state.feedback_repo = None

        from vilagent.persistence.thread_meta import make_thread_store
        ...
        if getattr(config.database, "backend", None) == "sqlite":
            from vilagent.utils.time import now_iso
...
def get_local_provider() -> LocalAuthProvider:
    ...
    if _cached_repo is None:
        from app.gateway.auth.repositories.sqlite import SQLiteUserRepository
        from vilagent.persistence.engine import get_session_factory
