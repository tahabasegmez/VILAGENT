# Contributing to VILAGENT Backend

Thank you for your interest in contributing to VILAGENT! This document provides guidelines and instructions for contributing to the backend codebase.
...
2. Clone your fork locally:
   ```bash
   git clone https://github.com/YOUR_USERNAME/vilagent.git
   cd vilagent
   ```
...
```python
import pytest
from vilagent.models.factory import create_chat_model
...
1. Create tool in `packages/harness/vilagent/tools/builtins/` or `packages/harness/vilagent/community/`:

```python
# packages/harness/vilagent/tools/builtins/my_tool.py
...
tools:
  - name: my_tool
    group: my_group
    use: vilagent.tools.builtins.my_tool:my_tool
```

### Adding New Middleware

1. Create middleware in `packages/harness/vilagent/agents/middlewares/`:

```python
# packages/harness/vilagent/agents/middlewares/my_middleware.py
...
2. Register in `packages/harness/vilagent/agents/lead_agent/agent.py`:
...
1. Update `packages/harness/vilagent/config/app_config.py` with new fields
...
Thank you for contributing to VILAGENT!
