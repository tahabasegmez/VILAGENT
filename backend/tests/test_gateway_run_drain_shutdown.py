...
Guards bytedance/vilagent issue #3373:
...
from vilagent.runtime import RunManager, RunStatus
...
async def test_langgraph_runtime_drains_runs_before_closing_checkpointer(monkeypatch):
    ...
    monkeypatch.setattr("vilagent.runtime.checkpointer.async_provider.make_checkpointer", probe_checkpointer)
    monkeypatch.setattr("vilagent.runtime.make_stream_bridge", fake_stream_bridge)
    monkeypatch.setattr("vilagent.runtime.make_store", fake_store)
    monkeypatch.setattr("vilagent.persistence.engine.init_engine_from_config", fake_init_engine)
    monkeypatch.setattr("vilagent.persistence.engine.close_engine", fake_close_engine)
    monkeypatch.setattr("vilagent.persistence.engine.get_session_factory", lambda: None)
    monkeypatch.setattr("vilagent.runtime.events.store.make_run_event_store", lambda _cfg: object())
    monkeypatch.setattr("vilagent.persistence.thread_meta.make_thread_store", lambda _sf, _store: object())
...
async def test_shutdown_surfaces_failed_interrupted_persist(caplog):
    ...
    try:
        await asyncio.wait_for(started.wait(), timeout=1.0)
        with caplog.at_level(logging.WARNING, logger="vilagent.runtime.runs.manager"):
            await rm.shutdown(timeout=5.0)
