...
spec = importlib.util.spec_from_file_location("vilagent_detect_uv_extras", DETECT_SCRIPT_PATH)
...
def isolated_cwd(tmp_path, monkeypatch):
    ...
    monkeypatch.delenv("VILAGENT_CONFIG_PATH", raising=False)
    return tmp_path
...
def test_resolve_extras_respects_explicit_config_path(tmp_path, monkeypatch):
    ...
    monkeypatch.chdir(tmp_path)
    monkeypatch.setenv("VILAGENT_CONFIG_PATH", str(elsewhere))
