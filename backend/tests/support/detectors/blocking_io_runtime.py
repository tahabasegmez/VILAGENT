"""Strict Blockbuster runtime context scoped to VILAGENT business code.

Creates a `BlockBuster` instance with `scanned_modules=("app", "vilagent")`
...
sync IO whose caller stack passes through `app.*` or `vilagent.*` raises
`BlockingError`.
...
_SCANNED_MODULES: tuple[str, ...] = ("app", "vilagent")

# Add VILAGENT-local rules here only when Blockbuster's default rule set misses
...
def detect_blocking_io_strict() -> Iterator[BlockBuster]:
    """Activate Blockbuster scoped to app.* and vilagent.* callers only."""
