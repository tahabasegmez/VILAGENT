...
Bytedance/vilagent issue #3107 BUG-001: the worker and lead-agent path
...
from vilagent.config.app_config import (
    AppConfig,
...
def _write_config_yaml(path: Path, *, log_level: str) -> None:
    path.write_text(
        f"""
sandbox:
  use: vilagent.sandbox.local.provider:LocalSandboxProvider
log_level: {log_level}
""".strip()
        + "\n",
        encoding="utf-8",
...
def test_get_config_reflects_file_mtime_reload(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("VILAGENT_CONFIG_PATH", str(config_file))
...
def test_get_config_returns_503_on_any_load_failure(monkeypatch, exception):
    ...
    Bytedance/vilagent issue #3107 BUG-001 review: the original snapshot
