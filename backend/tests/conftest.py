...
# Make 'app' and 'vilagent' importable from any working directory
sys.path.insert(0, str(Path(__file__).parent.parent))
sys.path.insert(0, str(Path(__file__).resolve().parents[2] / "scripts"))

# Break the circular import chain that exists in production code:
#   vilagent.subagents.__init__
#     -> .executor (SubagentExecutor, SubagentResult)
#       -> vilagent.agents.thread_state
#         -> vilagent.agents.__init__
#           -> lead_agent.agent
#             -> subagent_limit_middleware
#               -> vilagent.subagents.executor  <-- circular!
#
# By injecting a mock for vilagent.subagents.executor *before* any test module
...
sys.modules["vilagent.subagents.executor"] = _executor_mock


# Mock WindowsUIAPasswordRedactor._read_password_regions to avoid real UIA queries during tests
try:
...
# (see ``vilagent.runtime.user_context``). Without this fixture, every
...
def _reset_skill_storage_singleton():
    ...
    try:
        from vilagent.skills.storage import reset_skill_storage
    except ImportError:
        yield
        return
    reset_skill_storage()
...
def _restore_title_config_singleton():
    ...
    """
    try:
        from vilagent.config.title_config import reset_title_config
    except ImportError:
        yield
        return
...
def _auto_user_context(request):
    ...
    try:
        from vilagent.runtime.user_context import (
            reset_current_user,
            set_current_user,
        )
    except ImportError:
