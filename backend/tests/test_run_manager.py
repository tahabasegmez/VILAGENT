...
from vilagent.runtime import DisconnectMode, RunManager, RunStatus
from vilagent.runtime.runs.manager import PersistenceRetryPolicy
...
async def test_completion_persistence_warns_when_recreated_row_still_missing(caplog):
    ...
    record = await manager.create("thread-1")
    await manager.set_status(record.run_id, RunStatus.success)
    caplog.set_level(logging.WARNING, logger="vilagent.runtime.runs.manager")
...
async def test_list_by_thread_is_stable_when_timestamps_tie(manager: RunManager, monkeypatch: pytest.MonkeyPatch):
    ...
    monkeypatch.setattr("vilagent.runtime.runs.manager._now_iso", lambda: "2026-01-01T00:00:00+00:00")
...
async def test_model_name_create_or_reject():
    ...
    from vilagent.runtime.runs.schemas import DisconnectMode
