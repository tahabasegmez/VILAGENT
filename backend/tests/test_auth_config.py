...
def test_auth_config_missing_secret_generates_and_persists(tmp_path, caplog):
    ...
    from vilagent.config.paths import Paths
    ...
    try:
        with patch.dict(os.environ, {}, clear=True):
            ...
            with patch("vilagent.config.paths.get_paths", return_value=Paths(base_dir=tmp_path)), caplog.at_level(logging.WARNING):
                config = cfg.get_auth_config()
...
def test_auth_config_reuses_persisted_secret(tmp_path):
    ...
    try:
        with patch.dict(os.environ, {}, clear=True):
            os.environ.pop("AUTH_JWT_SECRET", None)
            with patch("vilagent.config.paths.get_paths", return_value=Paths(base_dir=tmp_path)):
                config = cfg.get_auth_config()
