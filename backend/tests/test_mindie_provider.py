...
from vilagent.models.mindie_provider import (
    MindIEChatModel,
    _fix_messages,
...
class TestMindIEInit:
    def test_timeout_kwargs_are_normalized(self):
        ...
        with patch("vilagent.models.mindie_provider.ChatOpenAI.__init__", new=fake_init):
            MindIEChatModel(
                model="mindie-test",
                api_key="test-key",
                connect_timeout=1.0,
...
class TestGenerate:
    def test_generate_calls_fix_messages_and_patch(self):
        with patch("vilagent.models.mindie_provider.ChatOpenAI._generate") as mock_super_gen, patch.object(MindIEChatModel, "__init__", return_value=None):
...
class TestAGenerate:
    ...
    async def test_agenerate_patches_result(self):
        with patch("vilagent.models.mindie_provider.ChatOpenAI._agenerate", new_callable=AsyncMock) as mock_ag, patch.object(MindIEChatModel, "__init__", return_value=None):
...
class TestAStream:
    ...
    async def test_no_tools_uses_real_stream(self):
        ...
        with patch("vilagent.models.mindie_provider.ChatOpenAI._astream", side_effect=fake_stream), patch.object(MindIEChatModel, "__init__", return_value=None):
            model = MindIEChatModel.__new__(MindIEChatModel)
