...
from vilagent.runtime.runs.manager import RunRecord
from vilagent.runtime.runs.schemas import DisconnectMode, RunStatus
from vilagent.runtime.runs.worker import RunContext, run_agent
...
def _clear_tracing_env(monkeypatch):
    from vilagent.config.tracing_config import reset_tracing_config
...
async def test_run_agent_falls_back_to_default_user_when_unset(monkeypatch):
    ...
    monkeypatch.setenv("LANGFUSE_SECRET_KEY", "sk-lf-test")
    from vilagent.config.tracing_config import reset_tracing_config
    from vilagent.runtime.runs import worker as worker_module
    from vilagent.runtime.user_context import DEFAULT_USER_ID
