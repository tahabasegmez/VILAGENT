...
def _real_subagent_executor() -> Iterator[type]:
    ...
    original_executor = sys.modules.get("vilagent.subagents.executor", _MISSING)
    parent_module = sys.modules.get("vilagent.subagents")
    original_parent_executor = getattr(parent_module, "executor", _MISSING) if parent_module is not None else _MISSING

    sys.modules.pop("vilagent.subagents.executor", None)
    for name in _EXECUTOR_IMPORT_MOCKS:
        sys.modules[name] = MagicMock()

    try:
        executor_module = importlib.import_module("vilagent.subagents.executor")
        yield executor_module.SubagentExecutor
    finally:
        if original_executor is _MISSING:
            sys.modules.pop("vilagent.subagents.executor", None)
        else:
            sys.modules["vilagent.subagents.executor"] = original_executor
...
async def test_load_skills_via_to_thread_does_not_block_event_loop(tmp_path: Path) -> None:
    ...
    from vilagent.subagents.config import SubagentConfig
