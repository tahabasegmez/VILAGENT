"""End-to-end demo: SafetyFinishReasonMiddleware on the real VILAGENT lead-agent.

What it proves
--------------
- The real ``make_lead_agent`` / ``VILAGENTClient`` pipeline is built (full
...
def main() -> int:
    ...
    # at import time via ``from vilagent.models import create_chat_model``,
    ...
    # imported names.
    import vilagent.agents.lead_agent.agent as lead_agent_module
    import vilagent.client as client_module
    ...
    from vilagent.client import VILAGENTClient

    try:
        client = VILAGENTClient()
