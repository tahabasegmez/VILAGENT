...
if [ -n "$VILAGENT_CONFIG_PATH" ] && [ -f "$VILAGENT_CONFIG_PATH" ]; then
    CONFIG="$VILAGENT_CONFIG_PATH"
...
MIGRATIONS = {
    1: {
        'description': 'Rename src.* module paths to vilagent.*',
        'replacements': [
            ('src.community.', 'vilagent.community.'),
            ('src.sandbox.', 'vilagent.sandbox.'),
            ('src.models.', 'vilagent.models.'),
            ('src.tools.', 'vilagent.tools.'),
        ],
    },
