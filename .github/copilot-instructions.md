# Copilot Onboarding Instructions for VILAGENT
...
VILAGENT is a full-stack "super agent harness".
...
- `backend/packages/harness/vilagent/agents/` - lead agent, middleware chain, memory
- `backend/app/gateway/` - FastAPI gateway API
- `backend/packages/harness/vilagent/sandbox/` - sandbox provider + tool wrappers
- `backend/packages/harness/vilagent/subagents/` - subagent registry/execution
- `backend/packages/harness/vilagent/mcp/` - MCP integration
- `backend/langgraph.json` - graph entrypoint (`vilagent.agents:make_lead_agent`)
