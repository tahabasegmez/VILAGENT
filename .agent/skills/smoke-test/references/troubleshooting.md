...
This document lists common issues encountered during VILAGENT smoke testing and how to resolve them.
...
3. Or stop VILAGENT services first:
   ```bash
   make stop
   ```
...
2. Check nginx logs:
   ```bash
   cd docker && docker compose -p vilagent-dev -f docker-compose-dev.yaml logs nginx
   ```
...
```bash
docker exec -it vilagent-gateway sh
```

#### Clean Up All VILAGENT-Related Containers and Images
```bash
make docker-stop
cd docker && docker compose -p vilagent-dev -f docker-compose-dev.yaml down -v
```
...
1. Check the GitHub issues for the project: https://github.com/bytedance/vilagent/issues
