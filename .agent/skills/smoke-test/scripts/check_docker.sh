...
if [ -n "$port_2026_usage" ]; then
    ...
    while IFS= read -r pid; do
        ...
        case "$process_command" in
            *[Dd]eer[Ff]low*|*[Dd]eerflow*|*[Nn]ginx*vilagent*|*vilagent/*[Nn]ginx*)
                vilagent_process_found=1
                ;;
        esac
    done <<EOF
...
EOF
    ...
    if [ "$vilagent_process_found" -eq 1 ]; then
        echo "✓ Port 2026 is occupied by VILAGENT"
    else
        echo "✗ Port 2026 must be free before starting VILAGENT"
        exit 1
    fi
else
