"""The managed web browser: a real Edge/Chrome (with the operator's profile) driven by Playwright.

The vision model sees viewport screenshots and its coordinates map 1:1 to page mouse
coordinates (device_scale_factor=1). One browser is shared across runs and stays open
after a task; it is closed on emergency stop and on shutdown.
"""
...
from vilagent.env.browser_profiles import resolve_profile
...
class PlaywrightBrowserSession:
    ...
    async def start(self) -> None:
        ...
        try:
            ...
            # Playwright starts its driver as a subprocess, which the Windows Selector event
            # loop cannot do (see __main__.loop_kwargs); the bare error says nothing useful.
            raise PlaywrightUnavailableError(
                "The browser driver could not start: this event loop cannot spawn processes. "
                "On Windows the gateway must run on the Proactor loop (vilagent.__main__ passes "
                'loop="none" to uvicorn).'
            ) from exc
        ...
        # A profile folder that does not exist would open an empty, signed-out profile that
        # looks like a guest session (Chrome has no "Default" when its profiles are numbered).
        profile_directory = resolve_profile(user_data_dir, profile_directory)
    ...
    async def run_action(self, action: Action) -> ActionOutcome:
        """Execute one model action on the page."""
        assert self._page is not None
        try:
            ...
            return ActionOutcome.failure(f"browser_unsupported_action:{action.kind.value}")
        except Exception as exc:
            return ActionOutcome.failure(f"browser_action_error:{exc.__class__.__name__}")

    @staticmethod
    def _point(action: Action) -> tuple[float, float] | None:
        point = action.point
        return (float(point[0]), float(point[1])) if point is not None else None

    async def _click(self, action: Action) -> ActionOutcome:
        point = self._point(action)
        if point is None:
            return ActionOutcome.failure("browser_click_missing_coordinate")
        x, y = point
    ...
    async def _type(self, action: Action) -> ActionOutcome:
    ...
    async def _keypress(self, action: Action) -> ActionOutcome:
        ...
        if isinstance(raw, str):
            ...
            tokens = []
        if not tokens:
            return ActionOutcome.failure("browser_keypress_missing_keys")
    ...
    async def _scroll(self, action: Action) -> ActionOutcome:
        try:
            pixels = float(action.args.get("amount") or action.args.get("pixels") or 0)
        except (TypeError, ValueError):
            pixels = 0.0
        if pixels == 0.0:
            # Default: scroll DOWN about one screen (x2 cancels the halving below).
            # FARA's convention is negative = down.
            pixels = -float(self._viewport_height) * 0.8 * 2
    ...
    async def _browser_action(self, action: Action) -> ActionOutcome:
        op = str(action.args.get("action") or "")
        if op == "visit_url":
            url = str(action.args.get("url") or "").strip()
            if not url:
                return ActionOutcome.failure("browser_visit_missing_url")
            if "://" not in url:
                url = "https://" + url
        ...
        if op == "web_search":
            ...
            if not query:
                return ActionOutcome.failure("browser_search_missing_query")
        ...
        if op == "refresh":
            ...
            await self._settle()
            return ActionOutcome.success()
        return ActionOutcome.failure(f"browser_unsupported_operation:{op or 'none'}")
...
class BrowserEnvironment(Environment):
    """The shared browser as an agent environment (started on first use)."""

    name = "browser"

    def __init__(self, config: ComputerUseBrowserConfig, control: Control):
        super().__init__(control)
        self._config = config
        self._session: PlaywrightBrowserSession | None = None

    def configure(self, config: ComputerUseBrowserConfig) -> None:
        """Use these settings from the next browser start (the operator changed the profile)."""
        self._config = config

    async def session(self) -> PlaywrightBrowserSession:
        config = self._config
        self._session = await get_shared_browser_session(
            headless=config.playwright_headless,
            viewport_width=config.viewport_width,
            viewport_height=config.viewport_height,
            channel=config.channel,
            use_user_profile=config.use_user_profile,
    ...
    @property
    def current_url(self) -> str:
        return self._session.current_url if self._session is not None else ""

    async def screenshot(self) -> bytes:
        return await (await self.session()).screenshot()

    async def _execute(self, action: Action) -> ActionOutcome:
        return await (await self.session()).run_action(action)
    ...
    def context_hint(self) -> str | None:
        ...
        return (
            "CURRENT PAGE: about:blank (a blank page). If this needs a website, FIRST navigate "
            "there with browser_action visit_url before anything else."
        )
